...
# --- multi-sine --------------------------------------------------------
function _generate_signal(::Val{:sin}, n::Int, dt::Float64, freq::Float64;
                          phase::Float64=0.0, noise_level::Float64=0.0, kwargs...)
    jvals  = 0:(2^n - 1)
    return [sin(freq * dt * j + phase) + noise_level * randn() for j in jvals]
end

function _generate_signal(::Val{:sin}, n::Int, dt::Float64, freq::Vector{Float64};
                          phase::Vector{Float64}=zeros(length(freq)), noise_level::Float64=0.0, kwargs...)
    jvals  = 0:(2^n - 1)
    length(freq) == length(phase) || throw(ArgumentError("Frequency and phase vectors must be of the same length."))
    return [sum(sin(ω * dt * j + φ) for (ω, φ) in zip(freq, phase)) + noise_level * randn() for j in jvals]
end

# --- pure random ------------------------------------------------------------
function _generate_signal(::Val{:random}, n::Int, dt::Float64;
...
function _generate_signal(::Val{:sin_decay}, n::Int, dt::Float64, freq::Float64;
                          decay_rate::Float64, phase::Float64=0.0, kwargs...)
    jvals  = 0:(2^n - 1)
    return [sin(freq * dt * j + phase) * exp(-decay_rate * dt * j) for j in jvals]
end

function _generate_signal(::Val{:sin_decay}, n::Int, dt::Float64, freq::Vector{Float64};
                          decay_rate::Vector{Float64}, phase::Union{Vector{Float64}, Nothing}=nothing, kwargs...)
    jvals  = 0:(2^n - 1)
    length(freq) == length(decay_rate) || throw(ArgumentError("Frequency and decay_rate vectors must be of the same length."))
    
    if phase === nothing
        return [sum(sin(ω * dt * j) * exp(-λ * dt * j) for (ω, λ) in zip(freq, decay_rate)) for j in jvals]
    else
        length(freq) == length(phase) || throw(ArgumentError("Frequency and phase vectors must be of the same length."))
        return [sum(sin(ω * dt * j + φ) * exp(-λ * dt * j) for (ω, λ, φ) in zip(freq, decay_rate, phase)) for j in jvals]
    end
end
...
function generate_signal(n::Int;
    ...
    elseif kind == :sin_decay
        ...
        throw(ArgumentError("Unsupported signal kind: $kind. Supported kinds are :sin, :sin_decay, :random."))
    end
end 
