...
function _generate_signal(
    ...
    n::Int,
    dt::Float64;
    n_terms::Int=_LEGACY_N_TERMS,
    seed_amp::Int=1001,
    seed_freq::Int=2002,
    ω_scale::Float64=40.0,
    kwargs...,
)
    N = 2^n
    jvals = 0:(N - 1)
    rng_ak = Xoshiro(seed_amp)
    rng_ωk = Xoshiro(seed_freq)
    ak = normalize(rand(rng_ak, n_terms))
    ωk = (ω_scale * dt) .* (rand(rng_ωk, n_terms) .- 0.5)
    return [sum(ak[k] * sin(ωk[k] * j) for k in 1:n_terms) for j in jvals]
end
...
function _generate_signal(
    ...
    rng_ak = Xoshiro(seed_amp)
    rng_ωk = Xoshiro(seed_freq)
    rng_λk = Xoshiro(seed_decay)
    ak = normalize(rand(rng_ak, n_terms))
    ωk = (ω_scale * dt) .* (rand(rng_ωk, n_terms) .- 0.5)
    λk = -(λ_scale * dt) .* rand(rng_λk, n_terms)
    return [sum(ak[k] * sin(ωk[k] * j) * exp(λk[k] * j) for k in 1:n_terms) for j in jvals]
end

function _generate_signal(::Val{:abs_cos_power_p8}, n::Int, dt::Float64; power::Float64=0.8, kwargs...)
    jvals = 0:(2^n-1)
    return [abs(cos(2π * dt * j))^power for j in jvals]
end
...
function generate_signal(
    ...
    # dt = 1 / (f * 2^n), using max(|f|) for multi-frequency signals.
    if isnothing(dt)
        if freq_typed isa Vector
            f_max = maximum(abs, freq_typed)
            dt_val = f_max == 0 ? 1.0 : (1.0 / (f_max * 2^n))
        else
            dt_val = freq_typed == 0 ? 1.0 : (1.0 / (abs(freq_typed) * 2^n))
        end
    else
        dt_val = Float64(dt)
    end
    ...
    elseif kind == :multi_sin
        return _generate_signal(Val(:multi_sin), n, dt_val; kwargs...)
    elseif kind == :sin_decay
        return _generate_signal(Val(:sin_decay), n, dt_val, freq_typed; kwargs...)
    elseif kind == :multi_sin_exp
        return _generate_signal(Val(:multi_sin_exp), n, dt_val; kwargs...)
    elseif kind == :abs_cos_power_p8
        return _generate_signal(Val(:abs_cos_power_p8), n, dt_val; kwargs...)
    else
        throw(
            ArgumentError(
                "Unsupported signal kind: $kind. Supported kinds are :sin, :multi_sin, :sin_decay, :multi_sin_exp, :abs_cos_power_p8, :random.",
            ),
        )
    end
end
