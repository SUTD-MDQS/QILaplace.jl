...
function zip_to_combine_mpos(mpo1::PairedSiteMPO, mpo2::PairedSiteMPO, oc::Int)
    ...
    if mpo1.sites_main[1] == mpo2.sites_main[1] # Zip-down (aligned at start)
        ...
        for k in 1:(2*L2)
            ...
            if k > 1
                ...
                # It should be in T, which is now in core
                # But we can also find it from new_data[k-1]
    ...
    return PairedSiteMPO(new_data, new_sites_main, new_sites_copy, new_bonds_main, new_bonds_copy), oc, direction
end


# Function to compress the zipped MPO starting from the given oc
function zip_to_compress_mpo(mpo::PairedSiteMPO, oc::Int, direction::String; cutoff=1e-14, maxdim=1000)
    ...
    elseif direction == "up"
        ...
        throw(ArgumentError("zip_to_compress_mpo: Unknown direction '$direction'. Must be 'up' or 'down'."))
    end

    return PairedSiteMPO(new_data, mpo.sites_main, mpo.sites_copy, new_bonds_main, new_bonds_copy), oc
end
