...
function zip_up_mpos(mpo1::SingleSiteMPO, mpo2::SingleSiteMPO, oc::Int)
    ...
    L1 > L2 || throw(
        ArgumentError(
            "zip_up_mpos: mpo1 must be longer than mpo2. Found length(mpo1)=$L1, length(mpo2)=$L2",
        ),
    )
    oc == L1 || throw(
        ArgumentError(
            "zip_up_mpos: Orthogonality center 'oc' must be at the end of mpo1 to do zip-up algorithm. Found oc=$oc, length(mpo1)=$L1",
        ),
    )
    ...
    for i_rev in 0:(L2 - 1)
        ...
        T, V = factorize(
            core_to_factorize, left_inds; ortho="right", tags=@sprintf("bond-%d", idx1 - 1)
        )
...
function build_qft_mpo(
    ...
    n ≥ 1 || throw(
        ArgumentError("build_qft_mpo: Number of qubits 'n' must be at least 1. Found n=$n"),
    )
    length(sites) == n || throw(
        ArgumentError(
            "build_qft_mpo: Number of sites must be equal to n. Found length(sites)=$(length(sites)), n=$n",
        ),
    )
