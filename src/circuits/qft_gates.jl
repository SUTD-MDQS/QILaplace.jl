...
function Π(i::Int, site_index::IType) where {IType<:Index}
...
function control_Hphase_mpo(k::Int, sites::Vector{IType}) where {IType<:Index}
    k ≥ 1 || throw(
        ArgumentError(
            "control_Hphase_mpo: Number of qubits 'k' must be at least 1. Found k=$k"
        ),
    )
    length(sites) == k || throw(
        ArgumentError(
            "control_Hphase_mpo: Number of sites must be equal to k. Found length(sites)=$(length(sites)), k=$k",
        ),
    )
    if k == 1
