...
function R(ωr::Real, site_index::IType) where {IType<:Index}
    Rmat = [
        1 0;
        0 exp(-ωr)
    ]
...
function control_damping_mpo(
    ...
    k ≥ 1 || throw(
        ArgumentError(
            "control_damping_mpo: Number of qubits 'k' must be at least 1. Found k=$k"
        ),
    )
    iseven(length(sites)) || throw(
        ArgumentError(
            "control_damping_mpo: Number of sites must be even. Found length(sites)=$(length(sites))",
        ),
    )
    length(sites) == 2k || throw(
        ArgumentError(
            "control_damping_mpo: Number of sites must be equal to 2k. Found length(sites)=$(length(sites)), k=$k",
        ),
    )
...
function control_damping_copy_mpo(
    ...
    k ≥ 1 || throw(
        ArgumentError(
            "control_damping_copy_mpo: Number of qubits 'k' must be at least 1. Found k=$k",
        ),
    )
    iseven(length(sites)) || throw(
        ArgumentError(
            "control_damping_copy_mpo: Number of sites must be even. Found length(sites)=$(length(sites))",
        ),
    )
    ...
    L = n - k + 1
    length(sites) == 2L || throw(
        ArgumentError(
            "control_damping_copy_mpo: Number of sites must be equal to 2*(n-k+1). Found length(sites)=$(length(sites)), n=$n, k=$k",
        ),
    )
    ...
    data[2] = (
        Π(0, sites_copy[1]) * onehot(bonds_copy[1], 1, bonds_main[1], 1) +
    ...
    # copy[L]: Identity
    data[2L] = (
        I(sites_copy[L]) * onehot(bonds_copy[L], 1) +
