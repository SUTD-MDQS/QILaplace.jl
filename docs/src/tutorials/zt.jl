...
p_coarse = scatter( #hide
    ...
    aspect_ratio=:equal, size=(750, 700), framestyle=:box, label=false, #hide
) #hide
savefig(p_coarse, joinpath(assets_dir, "zt_pole_scan_coarse.svg")) #hide
nothing #hide
...
scatter!(p_super, [real(z_target)], [imag(z_target)]; #hide
    marker=:circle, mc=:black, ms=7, msw=0, label="analytic pole location") #hide
    scatter!(p_super, [real(z_peak_s)], [imag(z_peak_s)]; #hide
    marker=:circle, mc=:white, msc=:black, ms=7, msw=1.5, label="numerical pole location") #hide
savefig(p_super, joinpath(assets_dir, "zt_pole_scan_superfine.svg")) #hide
nothing #hide
