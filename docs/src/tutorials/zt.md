...
````
amp_match = -0.2148648357954132
x[j_demo + 1] / x_norm = -0.2148648357954132
...
```math
\chi_{k,l} = \frac{1}{2^n}\sum_{j=0}^{N-1}\hat{x}_j\,
...
4×4 Matrix{ComplexF64}:
 0.1671+0.0im   0.273-0.1519im   0.164+0.0im   0.273+0.1519im
 0.2322+0.0im  0.2216-0.0166im  0.2017+0.0im  0.2216+0.0166im
 0.2225+0.0im  0.2194-0.0033im  0.2158+0.0im  0.2194+0.0033im
 0.2199+0.0im  0.2193-0.0007im  0.2186+0.0im  0.2193+0.0007im
````
...
x_hat = x ./ x_norm
χ_ref = ComplexF64[chi_analytical(x_hat, k, l, ωr, ωi) for k in 0:(N - 1), l in 0:(N - 1)]

@printf("max |χ_num - χ_ref| = %.3e\n", maximum(abs.(χ_num .- χ_ref)))

@printf("  %-12s  %-20s  %-20s  %6s\n", "(k,l)", "χ_num", "χ_exact", "|Δ|")
println("-"^74)
for k in 0:(N - 1), l in 0:(N - 1)
    ...
    @printf("  (%2d,%2d)      %-20s  %-20s  %8.2e\n", k, l, anum, aexact, abs(a - b))
end
...
````
max |χ_num - χ_ref| = 7.732e-16
  (k,l)         χ_num                 χ_exact                  |Δ|
--------------------------------------------------------------------------
  ( 0, 0)       0.16707+0.00000i      0.16707+0.00000i     3.78e-16
  ( 0, 1)       0.27297-0.15194i      0.27297-0.15194i     7.73e-16
  ( 0, 2)       0.16400+0.00000i      0.16400+0.00000i     1.01e-16
  ( 0, 3)       0.27297+0.15194i      0.27297+0.15194i     3.90e-16
  ( 1, 0)       0.23221+0.00000i      0.23221+0.00000i     4.18e-16
  ( 1, 1)       0.22157-0.01663i      0.22157-0.01663i     4.66e-16
  ( 1, 2)       0.20165+0.00000i      0.20165-0.00000i     1.76e-16
  ( 1, 3)       0.22157+0.01663i      0.22157+0.01663i     1.73e-16
  ( 2, 0)       0.22246+0.00000i      0.22246+0.00000i     6.55e-16
  ( 2, 1)       0.21935-0.00332i      0.21935-0.00332i     6.05e-16
  ( 2, 2)       0.21584+0.00000i      0.21584-0.00000i     4.20e-16
  ( 2, 3)       0.21935+0.00332i      0.21935+0.00332i     3.89e-16
  ( 3, 0)       0.21993+0.00000i      0.21993+0.00000i     2.82e-16
  ( 3, 1)       0.21925-0.00069i      0.21925-0.00069i     4.02e-16
  ( 3, 2)       0.21856+0.00000i      0.21856-0.00000i     5.94e-17
  ( 3, 3)       0.21925+0.00069i      0.21925+0.00069i     1.74e-16
...
````julia
function χ_finite_reference(z::ComplexF64, γp::ComplexF64, γm::ComplexF64, Nloc::Int, xnorm::Float64)
    s1 = (1 - (γp * z)^Nloc) / (1 - γp * z)
    s2 = (1 - (γm * z)^Nloc) / (1 - γm * z)
    return (0.5 / (Nloc * xnorm)) * (s1 + s2)
end
...
χ_ref_fine = ComplexF64[χ_finite_reference(z, γ_pos, γ_neg, N_big, xnorm_big) for z in zs_fine]
ΔM_fine = maximum(abs.(χ_fine .- χ_ref_fine))
@printf("fine scan max absolute coefficient error ΔM = %.3e\n", ΔM_fine)
...
fine scan max absolute coefficient error ΔM = 2.727e-09
...
ΔM_superfine = maximum(abs.(χ_super .- χ_finite_reference(z_peak_s, γ_pos, γ_neg, N_big, xnorm_big)))
...
@printf(" Maximum absolute coefficient error: %.3e\n", ΔM_superfine)
...
````
 ...
 Maximum absolute coefficient error: 1.347e-11
...
which we can see from the very small transform error (ΔM_superfine).
