...
class SVDRMPSConversion(Scene):
    def construct(self):
        ...
        s1_target = make_s_node(BOND_COLOR, "S_1").move_to(at_x(s1_x))
        s1 = s1_target.copy().stretch(S_SQUASH_Y, dim=1)
        self.add(s1)
        
        b_mid = make_bond(u1[0].get_right(), v1[0].get_left(), split=True)
        ...
        self.play(
            ...
            Transform(s1, s1_target), Create(b_mid),
            run_time=1.0,
            rate_func=smooth,
        )
        v1 = t_body
        ...
        # Truncate S1
        s1_light = make_s_node(COLOR_LIGHT_BLUE, "S_1").move_to(at_x(s1_x))

        b1_thin = make_bond(u1[0].get_right(), v1[0].get_left(), split=False)

        self.play(
            Transform(s1, s1_light),
            Transform(b_mid, b1_thin),
            run_time=0.6
        )
        ...
        b1_full.add_updater(lambda m: m.put_start_and_end_on(u1[0].get_right(), v1[0].get_left()))
        ...
        s2_target = make_s_node(BOND_COLOR, "S_2").move_to(at_x(s2_x))
        s2 = s2_target.copy().stretch(S_SQUASH_Y, dim=1)
        self.add(s2)
        
        # Create a single mid bond that spans from left tensor to right tensor
        b2_mid = make_bond(u2[0].get_right(), v2[0].get_left(), split=True)
        ...
        self.play(
            ...
            leg3.animate.shift(RIGHT * SPLIT_SHIFT),
            leg4.animate.shift(RIGHT * SPLIT_SHIFT),
            Transform(s2, s2_target), Create(b2_mid),
            *shift_Left_Step2,
            run_time=1.0,
            rate_func=smooth,
        )
        ...
        # Truncate S2
        s2_light = make_s_node(COLOR_LIGHT_BLUE, "S_2").move_to(at_x(s2_x))

        b2_thin = make_bond(u2[0].get_right(), v2[0].get_left(), split=False)

        self.play(
            Transform(s2, s2_light),
            Transform(b2_mid, b2_thin),
            run_time=0.6
        )
        ...
        b2_full.add_updater(lambda m: m.put_start_and_end_on(u2[0].get_right(), v2[0].get_left()))
        ...
        s3_target = make_s_node(BOND_COLOR, "S_3").move_to(at_x(s3_x))
        s3 = s3_target.copy().stretch(S_SQUASH_Y, dim=1)
        self.add(s3)
        
        # Create a single mid bond that spans from left tensor to right tensor
        b3_mid = make_bond(u3[0].get_right(), v3[0].get_left(), split=True)
        ...
        self.play(
            ...
            Transform(v2, v3),
            leg4.animate.shift(RIGHT * SPLIT_SHIFT),
            Transform(s3, s3_target), Create(b3_mid),
            *shift_Left_Step3,
            run_time=1.0,
            rate_func=smooth,
        )
        ...
        # Truncate S3
        s3_light = make_s_node(COLOR_LIGHT_BLUE, "S_3").move_to(at_x(s3_x))

        b3_thin = make_bond(u3[0].get_right(), v3[0].get_left(), split=False)

        self.play(
            Transform(s3, s3_light),
            Transform(b3_mid, b3_thin),
            run_time=0.6
        )
        ...
        b3_full.add_updater(lambda m: m.put_start_and_end_on(u3[0].get_right(), v3[0].get_left()))
