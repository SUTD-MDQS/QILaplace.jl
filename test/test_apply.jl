    ...
    @testset "Identity MPO application" begin
        ...
        for i in 1:n
            ...
            elseif i == n
                ...
                W_data[i] = delta(sp, s) * ITensor([1.0], mpo_bonds[i-1]) * ITensor([1.0], mpo_bonds[i])
            end
        end
    ...
    @testset "Single-site gate MPO" begin
        ...
        for i in 1:n
            ...
            elseif i == n
                ...
                # Bulk sites
                if i == 2  # Apply X gate
                    W_data[i] = ITensor(X, s', s) * ITensor([1.0], W_bonds[i-1]) * ITensor([1.0], W_bonds[i])
                else  # Identity
                    W_data[i] = delta(s', s) * ITensor([1.0], W_bonds[i-1]) * ITensor([1.0], W_bonds[i])
                end
            end
        end
    ...
    @testset "Site index mismatch" begin
        ...
        for i in 1:3
            ...
            elseif i == 3
                ...
                W_data[i] = delta(sp, s) * ITensor([1.0], W_bonds[i-1]) * ITensor([1.0], W_bonds[i])
            end
        end
...
@testset "ApplyMPO.jl: MPO composition" begin
    ...
    @testset "PairedSiteMPO composition via apply" begin
        ...
        data1 = ITensor[
            ...
            random_itensor(bonds_copy1[1], sites_copy[1], prime(sites_copy[1]), bonds_main1[1]),
            random_itensor(bonds_main1[1], sites_main[2], prime(sites_main[2]), bonds_copy1[2]),
        ...
        data2 = ITensor[
            ...
            random_itensor(bonds_copy2[1], sites_copy[1], prime(sites_copy[1]), bonds_main2[1]),
            random_itensor(bonds_main2[1], sites_main[2], prime(sites_main[2]), bonds_copy2[2]),
