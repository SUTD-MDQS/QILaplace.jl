...
@testset "mpo.jl: SingleSiteMPO struct validation" begin
    ...
    d = [random_itensor(s[1], s[1]', b[1]),
         random_itensor(s[2], s[2]', b[1])]
    @test_throws ArgumentError SingleSiteMPO(d, s, b)
    ...
    bad_d = [random_itensor(s[1], s[1]'),  # rank 2 instead of 3
             random_itensor(b[1], s[2], s[2]', b[2]),
    ...
    bad_d = [random_itensor(b[1]),  # missing site index
             random_itensor(b[1], s[2], s[2]', b[2]),
             random_itensor(b[2], s[3], s[3]')]
    @test_throws ArgumentError SingleSiteMPO(bad_d, s[1:3], b[1:2])
...
@testset "mpo.jl: PairedSiteMPO struct validation" begin
    ...
    bc = [Index(1, "bc1"), Index(1, "bc2")]
    d = [random_itensor(sm[1], sm[1]', bc[1]),
    ...
    bad_bc = [Index(1, "bad")]
    d_bad = [random_itensor(sm[1], sm[1]', bc[1]),
             random_itensor(sc[1], sc[1]', bc[1])]
    ...
    # Error if site index is missing
    bad_d = [random_itensor(bc[1]),  # missing site index
             random_itensor(sc[1], sc[1]', bc[1])]
    ...
    @test check_pairedsitempo(W.data, W.sites_main, W.sites_copy, W.bonds_main, W.bonds_copy) === nothing
end
