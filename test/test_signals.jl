...
@testset "Signals.jl: basic functionality" begin
    ...
    # 6. Error handling
    @test_throws ArgumentError generate_signal(n, kind=:sin_decay, freq=[1.0], decay_rate=[0.1, 0.2]) # Mismatch
...
@testset "Signals.jl: analytical correctness and randomness" begin
    ...
    s2 = generate_signal(n, kind=:sin, dt=dt, freq=[1.0,2.0], phase=[0.0,0.5], noise_level=0.0)
    expected2 = [sum(sin(ω * dt * j + φ) for (ω,φ) in zip([1.0,2.0],[0.0,0.5])) for j in 0:2^n-1]
    ...
    expected4 = [sum(sin(ω * dt * j) * exp(-λ * dt * j) for (ω,λ) in zip([1.0,2.0],[0.1,0.2])) for j in 0:2^n-1]
